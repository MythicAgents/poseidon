...
- Added the long-running `edit` command with Mythic GUI editing, UTF-8/2 MB validation, SHA-1 conflict detection, atomic replacement, editor-specific status updates, refresh, Done/close, and durable explicit force overwrite using the staged file ID.

### Changed

- Verified saved files by reading them back and reused the authoritative staged upload in the success response instead of transferring an identical post-save snapshot.

## [2.3.2] - 2026-05-11

### Changed

- Updated poseidon's build for garble to not try to resign the binary unless it's for macOS
...
- Massive change to restructure all commands as optional to mirror old PR#61 from paul-axe with some tweaks
  - each command is in its own package and is only included if the name of the command is passed in as a build tag
    - when included, commands self-register via their init functions into the `taskRegistrar`
  - each command has a "blank.go" file that is imported if the command is not included
  - a `modules.go` file is created for package `main` that imports all of these commands (or their blank files)
  - some commands have dependencies
    - `list_entitlements` uses `ps.Processes()`
- Some of the browserscripts have been updated to support the new `tabs` functionality
- Updated Poseidon's definition to allow "dynamic loading"
  - this is only at build time - no loading new commands at run time due to Go limitations
- Updated all the commands to be `Suggested` so that they're included by default
- `exit`, `jobs`, `jobkill` are "builtin" and can't be removed
- Updated `Makefile` builds to include `debug` tag which adds all commands

## [2.2.32] - 2026-03-12

### Changed

- Updated the garble build process to use the rust `rcodesign` utility to re-sign the binary after patching out "bad bytes"
- Updated `ls` browserscript to leverage tabs for multiple tables

## [2.2.31] - 2026-02-27

### Changed

- Fixed a typo in rpfwd that was `rpfwds` instead of the correct `rpfwd`

## [2.2.30] - 2026-02-20

### Changed

- Updated the `cd` command to still update the displayed `cwd` even if your current directory has permission issues
- Updated the `build` code to use arg arrays for exec.Command 

## [2.2.29] - 2026-02-16

### Changed

- Fixes for P2P performance

## [2.2.28] - 2026-02-02

### Changed

- Fixed some P2P tracking issues with 2+ hops

## [2.2.27] - 2026-01-02

### Changed

- Reworked the DNS profile a bit to make it easier to follow

## [2.2.26] - 2025-12-30

### Changed

- Updated `dns` profile for fallback TCP connection on truncated UDP requests
- Updated a few header files for ObjectiveC code
